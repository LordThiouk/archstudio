/* The icon set — the glyph a component wears in its scope chip.
 *
 * In `src/lib/` rather than beside the React component because it has three
 * consumers now: the editor's `<Icon>`, the icon pickers, and the SVG export,
 * which draws the same paths by hand into a file with no DOM to render them.
 * That export is the reason a database looks like a database on a printed sheet
 * and not only on screen.
 *
 * Shared with the viewer engine so that what you pick in the editor is exactly
 * what renders in the export. Keep the two in sync: the source of truth is the
 * ICON map in viewer/engine.js.
 *
 * Every path is drawn on a 24x24 viewBox, stroked, never filled — which is what
 * lets one glyph sit on a white card and on a scope-coloured chip alike.
 */
...
export const ICONS: Record<string, string> = {
  ...
  /* Two boxes sharing a width, one under the other — the gesture itself. */
  stack: '<rect x="4" y="4" width="16" height="6.5" rx="1.5"/><rect x="4" y="13.5" width="16" height="6.5" rx="1.5"/>',
...
/** The viewBox every glyph is drawn on. */
export const ICON_VIEWBOX = 24;

export const iconPath = (name: string | undefined): string =>
  ICONS[name || 'box'] || ICONS.box;
