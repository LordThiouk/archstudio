...
export const LEGO_CATALOG_VERSION = '2026-08-16.1';
...
export function ensureLegoCatalog(): void {
  if (db.prepare('SELECT 1 FROM lego_catalog_versions WHERE version=?').get(LEGO_CATALOG_VERSION)) return;
...
export function legoCatalog(lang: LegoLanguage = 'en'): LegoCatalogSnapshot {
  ...
  return { version, lang, scopes, aliases, bricks, intents, variants, technologyDescriptions };
}

export function legoCatalogCounts() {
  ensureLegoCatalog();
  return Object.fromEntries(['lego_scopes', 'lego_bricks', 'lego_intents', 'lego_variants', 'lego_technology_descriptions'].map(table => [table, plain<{ count: number }>(db.prepare(`SELECT count(*) AS count FROM ${table} WHERE catalog_version=?`).get(LEGO_CATALOG_VERSION)).count]));
}
